...
;--------------------------------
; Base sensor values and variance
BASE_LINE     FCB   $78
BASE_BOW      FCB   $D5
BASE_MID      FCB   $D6
BASE_PORT     FCB   $B1
...
LINE_VARIANCE           FCB   $10            
BOW_VARIANCE            FCB   $4C           
PORT_VARIANCE           FCB   $35                     
MID_VARIANCE            FCB   $35
...
HEX_TABLE       FCC   '0123456789ABCDEF'    ; Lookup for bits to ascii
BCD_SPARE     RMB   2
...
; subroutine section
;***************************************************************************************************
DISPATCHER        JSR   VERIFY_START                        ; Where dispatcher starts
                  RTS

VERIFY_START      CMPA  #START                              ; If it's the START state
                  BNE   VERIFY_FORWARD                      ; If not, check FWD
                  JSR   START_ST                            ; Run start state
                  RTS                                         

VERIFY_FORWARD    CMPA  #FWD                                ; If it's the FWD state
                  BNE   VERIFY_STOP                         ; If not, check ALL_STOP
                  JSR   FWD_ST                              ; Run fwd state
                  RTS
...
VERIFY_RIGHT_ALIGN CMPA  #RIGHT_ALIGN                       
                  ...
                  RTS                                       ; Exit state dispatcher

;***************************************************************************************************
; START state                                                                                      *
;***************************************************************************************************
START_ST          BRCLR   PORTAD0, %00000100,RELEASE          ; Stay when rear bumper is hit                         
                  JSR     INIT_FWD                            ; Move fwd after releasing                                   
                  MOVB    #FWD, CRNT_STATE
...
FWD_ST            BRSET   PORTAD0, $04, NO_FWD_BUMP           ; If doesn't bump, skip                           
                  MOVB    #REV_TRN, CRNT_STATE                ; Otherwise bow bumper hit                                
                                                                                            
                  JSR     UPDT_DISPL                          ; Display new state                                
                  JSR     INIT_REV                                                                
                  LDY     #6000                                                                   
                  JSR     del_50us                            ; Small reverse delay                                     
                  JSR     INIT_RIGHT                                                              
                  LDY     #6000                                                                   
                  JSR     del_50us                                                                
                  LBRA    EXIT                                                                    
...
ENABLE_TOF        LDAA    #%10000000
                  STAA    TSCR1           ; Enable timer counter
...
dataMov:          BSET  LCD_CNTR, LCD_E   ; pull LCD E-signal high
                  ...
                  BCLR  LCD_CNTR, LCD_E   ; pull the LCD E-signal low to complete write oper.
                  LDY   #1                ; adding this delay completes the internal
